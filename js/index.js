...
function initSectionScrollHandoff() {
    ...
    /*
     * 섹션 핸드오프 공통 계산기:
     * 1) wheel/touch가 같은 섹션 경계 규칙을 공유
     * 2) 입력 타입별 임계치만 분기해서 실기기/브라우저 체감을 최대한 일치시킨다.
     */
    ...
    const handleWheel = (event) => {
        ...
        const predictedScrollTop = Math.max(0, getPageScrollTop() + normalizedDeltaY * WHEEL_PREDICTION_GAIN);
    ...
    /*
     * 터치 제스처 상태머신:
     * tracking -> locked_vertical|locked_horizontal -> handoff
     * handoff가 시작되면 짧은 구간 preventDefault로 기본 관성 스크롤과 충돌을 줄인다.
     */
    ...
    const handleTouchMove = (event) => {
        ...
        if (touchGestureState === 'handoff') {
            if (event.cancelable && performance.now() <= touchPreventDefaultUntil) {
                event.preventDefault();
            }
            return;
        }
        ...
        if (touchGestureState === 'tracking') {
            if (
                gestureMetrics.absDeltaX < TOUCH_GESTURE_LOCK_MIN_DELTA_PX
                && gestureMetrics.absDeltaY < TOUCH_GESTURE_LOCK_MIN_DELTA_PX
            ) {
                return;
            }

            if (gestureMetrics.absDeltaY > gestureMetrics.absDeltaX * TOUCH_VERTICAL_LOCK_RATIO) {
                touchGestureState = 'locked_vertical';
            } else if (gestureMetrics.absDeltaX > gestureMetrics.absDeltaY * TOUCH_VERTICAL_LOCK_RATIO) {
                touchGestureState = 'locked_horizontal';
                return;
            } else {
                return;
            }
        ...
        if (touchGestureState === 'locked_horizontal') {
            return;
        }

        if (touchGestureState !== 'locked_vertical') {
            return;
        }

        tryTriggerTouchHandoff(gestureMetrics, event);
    };
    ...
    const handleTouchEnd = (event) => {
        ...
        if (touchGestureState !== 'handoff' && touchGestureState !== 'locked_horizontal') {
            const gestureMetrics = getTouchGestureMetrics();
            tryTriggerTouchHandoff(gestureMetrics);
        }
